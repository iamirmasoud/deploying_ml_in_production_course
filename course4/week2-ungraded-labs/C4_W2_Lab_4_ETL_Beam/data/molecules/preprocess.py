...
def run(
    ...
    if not isinstance(labels, list):
        raise ValueError(
            "`labels` must be list(str). " "Given: {} {}".format(labels, type(labels))
        )
    ...
    if not isinstance(feature_extraction, beam.PTransform):
        raise ValueError(
            ...
            "Given: {} {}".format(
                beam.PTransform, feature_extraction, type(feature_extraction)
            )
        )
    ...
    if not callable(feature_scaling):
        raise ValueError(
            ...
            "Given: {} {}".format(feature_scaling, type(feature_scaling))
        )

    if beam_options and not isinstance(beam_options, PipelineOptions):
        raise ValueError(
            "`beam_options` must be {}. "
            "Given: {} {}".format(PipelineOptions, beam_options, type(beam_options))
        )
    ...
    with beam.Pipeline(options=beam_options) as p, beam_impl.Context(
        temp_dir=tft_temp_dir
    ):
        ...
        train_dataset, eval_dataset = dataset | "Split dataset" >> beam.Partition(
        ...
        _ = train_dataset | "Write train dataset" >> tfrecordio.WriteToTFRecord(
            train_dataset_prefix, coder
        )

        eval_dataset_prefix = os.path.join(eval_dataset_dir, "part")
        _ = eval_dataset | "Write eval dataset" >> tfrecordio.WriteToTFRecord(
            eval_dataset_prefix, coder
        )

        # Write the transform_fn
        _ = transform_fn | "Write transformFn" >> transform_fn_io.WriteTransformFn(
            work_dir
        )
