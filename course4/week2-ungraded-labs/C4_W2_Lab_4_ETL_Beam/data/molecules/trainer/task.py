...
def _input_fn(
    file_pattern, tf_transform_output, label_key=None, num_epochs=None, batch_size=64
) -> tf.data.Dataset:
...
def _build_keras_model(hidden_units: List[int] = None) -> tf.keras.Model:
    ...
    model = dnn_regressor(dnn_hidden_units=hidden_units or [100, 70, 50, 25])
...
def run_fn(fn_args):
    ...
    train_dataset = _input_fn(
        fn_args.train_files_pattern, tf_transform_output, label_key, 10
    )
    eval_dataset = _input_fn(
        fn_args.eval_files_pattern, tf_transform_output, label_key, 10
    )
    ...
    # Train the model
    model.fit(train_dataset, steps_per_epoch=100, validation_data=eval_dataset)
    ...
    signatures = {
        "serving_default": _get_serve_tf_examples_fn(
            model, tf_transform_output, input_feature_spec
        ).get_concrete_function([signatures_dict])
    }
