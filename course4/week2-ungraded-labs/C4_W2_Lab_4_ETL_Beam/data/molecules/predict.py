...
class Predict(beam.DoFn):
    def __init__(self, model_dir, id_key):
    ...
    def process(self, inputs):
        ...
        # Reshape inputs to expected input shapes of the model
        for molecule in inputs:
            inputs[molecule] = tf.reshape(inputs[molecule], [1])

        # Pass inputs to serving signature to get predictions
        ...
        prediction = results["output_1"].numpy().tolist()[0]

        yield {"id": id_key, "prediction": prediction}
...
if __name__ == "__main__":
    ...
    run(args.model_dir, pubchem.SimpleFeatureExtraction(source), sink, beam_options)
