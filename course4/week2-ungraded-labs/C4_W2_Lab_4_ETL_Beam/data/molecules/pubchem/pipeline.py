...
class CountAtoms(beam.DoFn):
    def count_by_atom_symbol(self, molecule, atom_symbol):
        ...
        return sum(
            int(atom["atom_symbol"] == atom_symbol) for atom in molecule["atoms"]
        )
    ...
    def process(self, molecule):
        try:
            ...
            result = {
                "ID": tf.cast(uid, tf.int64),
                "TotalC": tf.cast(self.count_by_atom_symbol(molecule, "C"), tf.int64),
                "TotalH": tf.cast(self.count_by_atom_symbol(molecule, "H"), tf.int64),
                "TotalO": tf.cast(self.count_by_atom_symbol(molecule, "O"), tf.int64),
                "TotalN": tf.cast(self.count_by_atom_symbol(molecule, "N"), tf.int64),
                "Energy": tf.cast(label, tf.float32),
            }

            yield result
        except Exception as e:
            logging.info(
                "Invalid molecule, skipping: {}".format(molecule), exc_info=True
            )
